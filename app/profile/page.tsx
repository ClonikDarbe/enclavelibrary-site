  ...
  return <main className="profile-shell">
    ...
    <section className="profile-body">
      ...
      <div className="profile-columns">
        ...
        <section className="profile-panel"><p className="eyebrow"><span /> EN ÇOK OYNANANLAR</p><div className="profile-game-list">{games.slice(0, 8).map((game) => <article key={`${game.platform}:${game.title}`}>{game.cover_url ? <img src={game.cover_url} alt="" referrerPolicy="no-referrer" /> : <span>{initials(game.title)}</span>}<div><b>{game.title}</b><small>{game.platform} • {formatMinutes(game.playtime_minutes)}</small></div></article>)}</div>{recentGames.length > 0 && <><p className="eyebrow profile-recent-heading"><span /> SON OYNANANLAR</p><div className="profile-recent-list">{recentGames.map((game) => <span key={`${game.platform}:${game.title}:recent`}><b>{game.title}</b><small>{formatDate(game.last_played)}</small></span>)}</div></>}</section>
      </div>
    </section>
  </main>;
}
